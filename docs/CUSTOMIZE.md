...
This site has one canonical light-mode accent color in `_config.yml`:

```yaml
theme_color: "#3366CC"
```

Change only this quoted six-digit hex value and rebuild the site. The local `assets/css/main.scss` entrypoint passes it into the `al_folio_core` Sass tokens, so no `_sass` file needs editing. The Star History generator reads the same value; run `python3 bin/generate_star_history.py` if you also want to refresh its committed README SVGs.
